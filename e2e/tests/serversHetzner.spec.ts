...
test.describe("Servers: Hetzner", () => {
  ...
  /* ------------------------ Servers Scenarios ------------------------ */
  test("[SC014] createServer: Hetzner", { tag: ['@Server', '@Hetzner', '@Slow-TEST'] }, async () => {
    ...
    // Save the server ID
    testData.saveData({
      serverId1: serverId.toString()
    });
    console.log("Server ID: " + serverId);
  });

  test("[SC015] validateServer: Hetzner", { tag: ['@Server', '@Hetzner', '@Slow-TEST'] }, async () => {
    test.setTimeout(60 * 1000);
    const serversPage = new ServersPage(page);
    console.log(await serversPage.validateServerHetzner(serverName));
  });

  test("[SS045] createPermaSite1", { tag: ['@PermaSite', '@FreeDomain', '@Nginx', '@7.4', '@Slow-TEST'] }, async () => {
    const sitesPage = new SitesPage(page);
    [siteName, siteId] = await sitesPage.createSiteFreeDomainNginx(serverName, phpVersion7_4);
    // Save the site ID
    testData.saveData({
      nginxSiteId: siteId.toString()
    });
    console.log(siteName, siteId);
  });

  test("[SS046] validatePermaSite1", { tag: ['@PermaSite', '@FreeDomain', '@Nginx', '@7.4', '@Slow-TEST'] }, async () => {
    test.setTimeout(60 * 1000);
    const sitesPage = new SitesPage(page);
  ...
  test("[SS047] createPermaSite2", { tag: ['@PermaSite', '@FreeDomain', '@Ols', '@7.4', '@Slow-TEST'] }, async () => {
    const sitesPage = new SitesPage(page);
    [siteName, siteId] = await sitesPage.createSiteFreeDomainOls(serverName, phpVersion7_4);

    testData.saveData({
      olsSiteId: siteId.toString()
    });
    console.log(siteName, siteId);
  });

  test("[SS048] validatePermaSite2", { tag: ['@PermaSite', '@FreeDomain', '@Ols', '@7.4', '@Slow-TEST'] }, async () => {
    test.setTimeout(60 * 1000);
    const sitesPage = new SitesPage(page);
